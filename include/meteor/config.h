...
typedef struct {
  ...
  char server_ip /* flawfinder: ignore */[64];  /* -S, N100 receiver IP, default
                                                   192.168.1.100 */
  char station_id /* flawfinder: ignore */[20]; /* -I, RMS station ID,   default
                                                   XX0001 */
  /* Storage management */
  ...
  int retention_days;   /* -R, delete events older than N days, 0=off, default 7
                         */
  int color;            /* -C, capture color JPEGs, default 0 */
} meteor_config;
