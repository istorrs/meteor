...
static int64_t elapsed_ms(const struct timespec *from,
...
static int make_event_dir(meteor_event_ctx *ctx) {
  ...
  (void)snprintf(ctx->event_dir, sizeof(ctx->event_dir),
                 "%s/%04d%02d%02d_%02d%02d%02d", ctx->cfg->output_dir,
                 tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
                 tm.tm_min, tm.tm_sec);

  ret = mkdir(ctx->event_dir, 0700);
  if (ret && errno != EEXIST) {
    METEOR_LOG_ERR("mkdir(%s) failed: %s", ctx->event_dir, strerror(errno));
    return -1;
  }
...
meteor_event_state meteor_event_update(meteor_event_ctx *ctx,
  ...
  if (result->triggered > 0) {
    ...
    case METEOR_EVENT_IDLE:
      ...
      METEOR_LOG_INFO("event started: %s (%d ROIs triggered)", ctx->event_dir,
                      result->triggered);
      break;
    case METEOR_EVENT_COOLDOWN:
      ctx->state = METEOR_EVENT_ACTIVE;
      METEOR_LOG_DBG("event reactivated (%d ROIs)", result->triggered);
      break;
...
static int remove_event_dir(const char *path) {
  char filepath[1024]; /* flawfinder: ignore */
  DIR *d;
  struct dirent *ent;
  int rc = 0;
  ...
  while ((ent = readdir(d)) != NULL) {
    if (ent->d_name[0] == '.')
      continue;
    if (snprintf(filepath, sizeof(filepath), "%s/%s", path, ent->d_name) >=
        (int)sizeof(filepath))
      continue;
...
void meteor_event_cleanup_old(const meteor_config *cfg) {
  ...
  while ((ent = readdir(d)) != NULL) {
    ...
    (void)snprintf(path, sizeof(path), "%s/%s", cfg->output_dir, ent->d_name);
  ...
  if (removed > 0)
    METEOR_LOG_INFO("cleanup: removed %d old event director%s", removed,
                    removed == 1 ? "y" : "ies");
}
