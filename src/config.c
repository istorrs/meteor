...
void meteor_config_defaults(meteor_config *cfg) {
  ...
  (void)snprintf(cfg->server_ip, sizeof(cfg->server_ip), "%s",
                 DETECTOR_DEFAULT_SERVER_IP);
  (void)snprintf(cfg->station_id, sizeof(cfg->station_id), "%s",
                 DETECTOR_DEFAULT_STATION_ID);
  cfg->station_id[sizeof(cfg->station_id) - 1] = '\0';
...
static void usage(const char *prog) {
  (void)fprintf(
      ...
      "  -C        Capture color JPEGs instead of grayscale\n"
      "  -h        Show this help\n",
      prog, DEFAULT_SENSITIVITY, DEFAULT_GRID_COLS, DEFAULT_GRID_ROWS,
      DEFAULT_COOLDOWN_SECS, DEFAULT_CAPTURE_INTERVAL, DEFAULT_OUTPUT_DIR,
...
int meteor_config_parse(meteor_config *cfg, int argc, char **argv) {
  ...
  while ((opt = getopt /* flawfinder: ignore */ (
              argc, argv, "s:g:r:c:f:o:S:I:m:R:Ch")) != -1) {
    switch (opt) {
    case 's':
      cfg->sensitivity = (int)strtol(optarg, &endptr, 10);
      if (endptr == optarg || *endptr != '\0' || cfg->sensitivity < 0 ||
          cfg->sensitivity > 4) {
        (void)fprintf(stderr, "sensitivity must be 0-4\n");
        return -1;
      }
      break;
    case 'g':
      cfg->grid_cols = (int)strtol(optarg, &endptr, 10);
      if (endptr == optarg || *endptr != '\0' || cfg->grid_cols < 1) {
        (void)fprintf(stderr, "grid columns must be >= 1\n");
        return -1;
      }
      break;
    case 'r':
      cfg->grid_rows = (int)strtol(optarg, &endptr, 10);
      if (endptr == optarg || *endptr != '\0' || cfg->grid_rows < 1) {
        (void)fprintf(stderr, "grid rows must be >= 1\n");
        return -1;
      }
      break;
    case 'c':
      cfg->cooldown_secs = (int)strtol(optarg, &endptr, 10);
      if (endptr == optarg || *endptr != '\0' || cfg->cooldown_secs < 1) {
        (void)fprintf(stderr, "cooldown must be >= 1\n");
        return -1;
      }
      break;
    case 'f':
      cfg->capture_interval_ms = (int)strtol(optarg, &endptr, 10);
      if (endptr == optarg || *endptr != '\0' || cfg->capture_interval_ms < 0) {
        (void)fprintf(stderr, "capture interval must be >= 0\n");
        return -1;
      }
      break;
    case 'o':
      ...
      break;
    case 'S':
      (void)snprintf(cfg->server_ip, sizeof(cfg->server_ip), "%s", optarg);
      break;
    case 'I':
      (void)snprintf(cfg->station_id, sizeof(cfg->station_id), "%s", optarg);
      break;
    case 'm':
      cfg->max_event_frames = (int)strtol(optarg, NULL, 10);
      if (cfg->max_event_frames < 0) {
        (void)fprintf(stderr, "max event frames must be >= 0\n");
        return -1;
      }
      break;
    case 'R':
      ...
      if (cfg->retention_days < 0) {
        (void)fprintf(stderr, "retention days must be >= 0\n");
        return -1;
      }
      break;
    case 'C':
