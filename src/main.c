...
static int ensure_output_dir(const char *path) {
  ...
  if (stat(path, &st) == 0) {
    if (!S_ISDIR(st.st_mode)) {
      METEOR_LOG_ERR("path exists but is not a directory: %s", path);
      return -1;
    }
    return 0;
  }
...
int main(int argc, char **argv) {
  ...
  METEOR_LOG_INFO(
      "config: sense=%d grid=%dx%d cooldown=%ds capture=%dms out=%s",
      cfg.sensitivity, cfg.grid_cols, cfg.grid_rows, cfg.cooldown_secs,
  ...
  /* 5. IVS */
  ret = meteor_ivs_init(IVS_GRP, IVS_CHN, WIDTH, HEIGHT, cfg.sensitivity,
