...
/* Color JPEG writing delegated to shared meteor_jpeg_write_nv12() */
...
static int isp_init(void) {
  ...
  (void)snprintf(sensor_info.name, sizeof(sensor_info.name), "%s",
                 detected_sensor);
  sensor_info.cbus_type = TX_SENSOR_CONTROL_INTERFACE_I2C;
  (void)snprintf(sensor_info.i2c.type, sizeof(sensor_info.i2c.type), "%s",
                 detected_sensor);
...
static int isp_configure_for_stacking(int exposure_secs, int grayscale) {
   ...
   * Non-fatal — we just get shorter sub-exposures per frame. */
  {
    ...
    ret = IMP_ISP_Tuning_SetSensorFPS(1, (uint32_t)exposure_secs);
    if (ret == 0) {
      LOG_INFO("sensor FPS set to 1/%d (%ds/frame)", exposure_secs,
               exposure_secs);
      ok = 1;
    } else {
      LOG_WARN("SetSensorFPS(1/%d) failed, trying "
               ...
               exposure_secs);
      for (i = 0; i < 6; i++) {
        ret = IMP_ISP_Tuning_SetSensorFPS(fps_table[i][0], fps_table[i][1]);
        if (ret == 0) {
          LOG_INFO("sensor FPS set to %u/%u "
                   "(~%ums/frame)",
                   fps_table[i][0], fps_table[i][1],
                   1000 * fps_table[i][1] / fps_table[i][0]);
          ok = 1;
          break;
        }
      }
   ...
   * deliver the requested exposure time. */
  {
    ...
    if (IMP_ISP_Tuning_GetExpr(&expr) == 0)
      LOG_INFO("integration time: cur=%u min=%u max=%u "
               "(%uus/line)",
               expr.g_attr.integration_time, expr.g_attr.integration_time_min,
               expr.g_attr.integration_time_max,
               expr.g_attr.one_line_expr_in_us);

    if (IMP_ISP_Tuning_GetSensorFPS(&fps_n, &fps_d) == 0 && fps_n > 0) {
      unsigned int frame_ms = 1000 * fps_d / fps_n;
      unsigned int req_ms = (unsigned int)exposure_secs * 1000;

      if (frame_ms < req_ms)
        LOG_WARN("-e %d requested %ums/frame but "
                 "sensor minimum is %u/%u FPS "
                 "(%ums/frame); using %ums",
                 exposure_secs, req_ms, fps_n, fps_d, frame_ms, frame_ms);
    }
  }

  LOG_INFO("ISP configured: night mode, no denoise, %ds target", exposure_secs);
  return 0;
}
...
static int load_dark(const char *dark_path, int grayscale, uint8_t **y_dark_out,
                     uint8_t **uv_dark_out) {
  ...
  *uv_dark_out = NULL;
  ...
  if ((size_t)st.st_size == color_size)
    ...
    LOG_WARN("dark frame %s has unexpected size %ld "
             "(expected %zu or %zu)",
             dark_path, (long)st.st_size, color_size, gray_size);
    return -1;
  }
  ...
  *y_dark_out = malloc((size_t)y_size);
  ...
  if (!grayscale && has_uv) {
    *uv_dark_out = malloc((size_t)uv_size);
    ...
    if (fread(*uv_dark_out, 1, (size_t)uv_size, fp) != (size_t)uv_size) {
      free(*y_dark_out);
      free(*uv_dark_out);
      *y_dark_out = NULL;
      *uv_dark_out = NULL;
...
static int stack_frames(int num_frames, int grayscale, int clip,
  ...
  uint8_t *y_avg = NULL;
  uint8_t *uv_avg = NULL;
  /* Meteor compositing needs max value */
  uint8_t *y_max = NULL;
  uint8_t *uv_max = NULL;
  /* Sigma clipping needs sum of squares and per-pixel valid counts */
  uint32_t *y_sq = NULL;
  uint32_t *uv_sq = NULL;
  uint16_t *y_count = NULL;
  uint16_t *uv_count = NULL;
  uint8_t **frames = NULL;
  int i, ret;
  ...
  /* Allocate max arrays for meteor compositing */
  if (meteor_thresh) {
  ...
  /* Allocate sigma clipping buffers and frame pointers */
  if (clip) {
    /* Rough check for 80% of typical 64MB RAM limit, ~50MB max for frames */
    size_t frame_bytes = (size_t)y_size + (grayscale ? 0 : (size_t)uv_size);
    if ((size_t)num_frames * frame_bytes > (size_t)50 * 1024 * 1024) {
      LOG_WARN("too many frames (%d) for sigma clipping RAM buffer, disabling "
               "clipping",
               num_frames);
      clip = 0;
    } else {
      y_sq = calloc((size_t)y_size, sizeof(uint32_t));
      y_count = calloc((size_t)y_size, sizeof(uint16_t));
      frames = (uint8_t **)calloc((size_t)num_frames, sizeof(uint8_t *));
      if (!grayscale) {
        uv_sq = calloc((size_t)uv_size, sizeof(uint32_t));
        uv_count = calloc((size_t)uv_size, sizeof(uint16_t));
      }

      if (!y_sq || !y_count || !frames ||
          (!grayscale && (!uv_sq || !uv_count))) {
        LOG_WARN("failed to allocate clipping buffers, falling back to simple "
                 "average");
        clip = 0;
      }
    }
  ...
  for (i = 0; i < num_frames && running; i++) {
    ...
    if (meteor_thresh && i > 0) {
      ...
      for (j = 0; j < y_size; j++) {
        ...
        if ((uint32_t)data[j] * (uint32_t)i >
            y_acc[j] + (uint32_t)meteor_thresh * (uint32_t)i)
          bright++;
      }
    ...
    for (j = 0; j < y_size; j++) {
      ...
      if (y_dark) {
        int d = (int)v - (int)y_dark[j];
        v = (d < 0) ? 0 : (uint8_t)d;
      }

      y_acc[j] += v;
      if (clip) {
        y_sq[j] += (uint32_t)v * (uint32_t)v;
      }
      if (meteor_thresh) {
        if (v > y_max[j])
          y_max[j] = v;
    ...
    if (!grayscale) {
      for (j = 0; j < uv_size; j++) {
        ...
        if (uv_dark) {
          int d = (int)v - (int)uv_dark[j];
          v = (d < 0) ? 0 : (uint8_t)d;
        }

        uv_acc[j] += v;
        if (clip) {
          uv_sq[j] += (uint32_t)v * (uint32_t)v;
        }
        if (meteor_thresh) {
          if (v > uv_max[j])
            uv_max[j] = v;
    ...
    if (clip && frames) {
      size_t frame_bytes = (size_t)y_size + (grayscale ? 0 : (size_t)uv_size);
      frames[frames_buffered] = malloc(frame_bytes);
      if (frames[frames_buffered]) {
        /* flawfinder: ignore */
        memcpy(frames[frames_buffered], data, frame_bytes);
        frames_buffered++;
      } else {
        LOG_WARN("failed to buffer frame %d, disabling clipping", i);
        clip = 0;
      }
    }
...

    {
      ...
      if (IMP_ISP_Tuning_GetEVAttr(&ev_attr) == 0)
        LOG_INFO("  frame %d/%d  fps=%u/%u"
                 ...
                 i + 1, num_frames, fps_n, fps_d, ev_attr.expr_us,
                 ev_attr.again, ev_attr.dgain);
      else
        LOG_INFO("  captured frame %d/%d", i + 1, num_frames);
    }
  }
  ...
  if (clip && num_frames < 4) {
    LOG_WARN("only %d frames — disabling outlier rejection", num_frames);
    clip = 0;
  }

  /* Average — with or without sigma clipping */
  if (clip) {
    LOG_INFO("averaging %d frames (2-pass sigma clipping)...", num_frames);
    for (i = 0; i < y_size; i++) {
      uint32_t mean = y_acc[i] / (uint32_t)num_frames;
      uint32_t var = (y_sq[i] / (uint32_t)num_frames) - (mean * mean);
      uint32_t thresh_sq = 4 * var;
      uint32_t sum = 0;
      uint16_t count = 0;
      for (int k = 0; k < frames_buffered; k++) {
        uint8_t v = frames[k][i];
        uint32_t diff = (v > mean) ? (v - mean) : (mean - v);
      ...
      if (count > 0)
        y_avg[i] = (uint8_t)((sum + (count / 2)) / count);
      else
        y_avg[i] = (uint8_t)mean;
    }
    if (!grayscale) {
      for (i = 0; i < uv_size; i++) {
        uint32_t mean = uv_acc[i] / (uint32_t)num_frames;
        uint32_t var = (uv_sq[i] / (uint32_t)num_frames) - (mean * mean);
        uint32_t thresh_sq = 4 * var;
        uint32_t sum = 0;
        uint16_t count = 0;
        for (int k = 0; k < frames_buffered; k++) {
          uint8_t v = frames[k][y_size + i];
          uint32_t diff = (v > mean) ? (v - mean) : (mean - v);
          if (diff * diff <= thresh_sq) {
            sum += v;
            count++;
        ...
        if (count > 0)
          uv_avg[i] = (uint8_t)((sum + (count / 2)) / count);
        else
          uv_avg[i] = (uint8_t)mean;
      }
    }
  } else {
    ...
    if (!grayscale) {
      for (i = 0; i < uv_size; i++)
        uv_avg[i] = (uint8_t)(uv_acc[i] / (uint32_t)num_frames);
    }
  }
    ...
    if (grayscale) {
      ...
      for (r = 0; r < HEIGHT; r++) {
        for (x = 0; x < WIDTH; x++) {
          ...
          if (y_max[yi] - y_avg[yi] > meteor_thresh) {
            int uvi = (r / 2) * WIDTH + (x & ~1);

            y_avg[yi] = y_max[yi];
            uv_avg[uvi] = uv_max[uvi];
            uv_avg[uvi + 1] = uv_max[uvi + 1];
            composited++;
          }
        }
      }
  ...
  if (y_dark) {
    ...
    if (!grayscale && uv_dark) {
      for (i = 0; i < uv_size; i++) {
        ...
        uv_avg[i] = (uint8_t)(v < 0 ? 0 : (v > 255 ? 255 : v));
      }
    }
  }
  ...
  /* Write output */
  if (grayscale)
    ret = meteor_jpeg_write_gray(output, y_avg, WIDTH, HEIGHT, quality);
  else
    ret = meteor_jpeg_write_nv12(output, y_avg, uv_avg, WIDTH, HEIGHT, quality);

  if (ret == 0)
    LOG_INFO("wrote %s (%dx%d, %d frames stacked%s%s)", output, WIDTH, HEIGHT,
             num_frames, clip ? ", clipped" : "",
             meteor_thresh ? ", meteor" : "");

cleanup:
  if (frames) {
    for (i = 0; i < frames_buffered; i++)
      free(frames[i]);
    free((void *)frames);
  }
  free(y_sq);
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "n:e:o:d:t:q:m:gcD:S:h")) != -1) {
    ...
    case 'n':
      ...
      if (endptr == optarg || *endptr != '\0' || num_frames < 1) {
        (void)fprintf(stderr, "invalid frame count\n");
        return 1;
      }
      break;
    case 'e':
      exposure = (int)strtol(optarg, &endptr, 10);
      if (endptr == optarg || *endptr != '\0' || exposure < 1 ||
          exposure > 30) {
        (void)fprintf(stderr, "exposure must be 1-30 seconds\n");
        return 1;
      }
      break;
    case 'o':
    ...
    case 'd':
      ...
      break;
    case 't':
      timelapse = (int)strtol(optarg, &endptr, 10);
      if (endptr == optarg || *endptr != '\0' || timelapse < 1) {
        (void)fprintf(stderr, "timelapse interval must be >= 1 second\n");
        return 1;
      }
      break;
    case 'q':
      quality = (int)strtol(optarg, &endptr, 10);
      if (endptr == optarg || *endptr != '\0' || quality < 1 || quality > 100) {
        (void)fprintf(stderr, "quality must be 1-100\n");
        return 1;
      }
      break;
    case 'g':
      ...
      break;
    case 'm':
      meteor_thresh = (int)strtol(optarg, &endptr, 10);
      if (endptr == optarg || *endptr != '\0' || meteor_thresh < 1 ||
          meteor_thresh > 255) {
        (void)fprintf(stderr, "meteor threshold must be 1-255\n");
        return 1;
      }
      break;
    case 'c':
  ...
  /* 5. Removed dark capture mode for STACK-3 */
  ...
  if (timelapse) {
    ...
    if (IMP_ISP_Tuning_GetSensorFPS(&fps_n, &fps_d) == 0 && fps_n > 0) {
      ...
      num_frames = (int)(fps_n * (uint32_t)timelapse / fps_d);
      if (num_frames < 4)
        num_frames = 4;
      ...
      LOG_WARN("cannot read sensor FPS, using -n %d", num_frames);
    }
  }
  ...
  /* 7. Load dark frame if provided */
  {
    uint8_t *y_dark = NULL;
    uint8_t *uv_dark = NULL;

    if (dark_path_arg) {
      if (load_dark(dark_path_arg, grayscale, &y_dark, &uv_dark) == 0)
        LOG_INFO("loaded dark frame from %s", dark_path_arg);
      else
        LOG_WARN("failed to load dark frame from %s", dark_path_arg);
    }
    ...
    if (timelapse)
      ...
      LOG_INFO("astrostack starting: %d x %ds subs -> %s/%s", num_frames,
               exposure, output_dir, output);
    ...
    if (timelapse) {
      ...
      while (running) {
        int n = snprintf(path_buf, sizeof(path_buf), "%s/timelapse-%06d.jpg",
                         output_dir, frame_idx);
        if (n < 0 || (size_t)n >= sizeof(path_buf)) {
          LOG_ERR("timelapse path truncated or error");
          ret = 1;
          break;
        }
      ...
      int n = snprintf(path_buf, sizeof(path_buf), "%s/%s", output_dir, output);
      if (n < 0 || (size_t)n >= sizeof(path_buf)) {
        LOG_ERR("output path truncated or error");
        ret = 1;
      } else {
        ret = stack_frames(num_frames, grayscale, clip, meteor_thresh, y_dark,
                           uv_dark, path_buf, quality);
