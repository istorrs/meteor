...
static void downsample_y(const uint8_t *src, int src_w, int src_h,
  ...
  for (dy = 0; dy < dst_h; dy++) {
    size_t sy = (size_t)dy * (size_t)y_step;
    const uint8_t *src_row = src + (ptrdiff_t)sy * (ptrdiff_t)src_stride;
...
static void sync_time_from_server(const PushConfig *push) {
  ...
  if (inet_pton(AF_INET, push->server_ip, &addr.sin_addr) != 1) {
    METEOR_LOG_WARN("time_sync: bad server IP '%s'", push->server_ip);
    (void)close(fd);
    return;
  }

  if (connect(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    METEOR_LOG_WARN("time_sync: cannot connect to %s:%d", push->server_ip,
                    push->server_port);
    (void)close(fd);
    return;
  }
  ...
  METEOR_LOG_INFO("time_sync: clock set to %lld UTC (from %s:%d)", unix_sec,
                  push->server_ip, push->server_port);
}
...
static void derive_station_id_from_mac(char *buf, size_t size) {
  ...
  for (i = 0; ifaces[i]; i++) {
    ...
    /* Parse "aa:bb:cc:dd:ee:ff" using strtoul per octet */
    p = mac;
    ...
    if (ok) {
      ...
      METEOR_LOG_INFO("station ID derived from %s MAC: %s", ifaces[i], buf);
      return;
    }
  }
...
static void usage(const char *prog) {
  (void)fprintf(stderr,
                ...
                "(default: %s)\n"
                "  -h       Show this help\n",
                prog, DETECTOR_DEFAULT_SERVER_IP, DEFAULT_STACK_SECS,
                DEFAULT_JPEG_QUALITY, DEFAULT_IVS_SENSE, DEFAULT_DARK_PATH);
}
...
int main(int argc, char **argv) {
  ...
  (void)snprintf(dark_path, sizeof(dark_path), "%s", DEFAULT_DARK_PATH);
  station_id[0] = '\0'; /* filled by -I or derive_station_id_from_mac() */

  while ((opt = getopt /* flawfinder: ignore */ (argc, argv,
                                                 "S:I:t:q:s:d:h")) != -1) {
    switch (opt) {
    case 'S':
      ...
      break;
    case 'I':
      (void)snprintf(station_id, sizeof(station_id), "%s", optarg);
      station_id_set = 1;
      break;
    case 't':
      stack_secs = (int)strtol(optarg, NULL, 10);
      if (stack_secs < 1) {
        (void)fprintf(stderr, "stack interval must be >= 1\n");
        return 1;
      }
      break;
    case 'q':
    ...
    case 's':
      ...
      break;
    case 'd':
      (void)snprintf(dark_path, sizeof(dark_path), "%s", optarg);
      break;
    case 'h':
      usage(argv[0]);
      return 0;
  ...
  METEOR_LOG_INFO("nightcam starting: server=%s station=%s "
                  ...
                  server_ip, station_id, stack_secs, jpeg_quality, ivs_sense);
  ...
  (void)snprintf(ff_hdr.station_id, sizeof(ff_hdr.station_id), "%s",
                 station_id);
  ...
  stk = stacker_create(&push, station_id, stack_secs * FPS, jpeg_quality,
                       dark_path);
  if (!stk) {
  ...
  ret = meteor_ivs_init(IVS_GRP, IVS_CHN, WIDTH, HEIGHT, ivs_sense,
                        IVS_GRID_COLS, IVS_GRID_ROWS);
  if (ret)
    goto err_fs;
  ...
  while (running) {
    ...
    downsample_y(data, WIDTH, HEIGHT, WIDTH, detect_buf, DETECT_WIDTH,
                 DETECT_HEIGHT);
