#!/usr/bin/env python3
# pylint: disable=missing-function-docstring,missing-class-docstring,too-many-lines,too-many-instance-attributes,invalid-name,broad-exception-caught,too-many-arguments,too-many-positional-arguments,too-many-locals,too-many-branches,too-many-statements,consider-using-with,chained-comparison,import-outside-toplevel,multiple-statements,too-few-public-methods,no-member
...
class NightcamReview:
    ...
    def _display_file(self, path: str):
        ...
        try:
            json_path = os.path.splitext(path)[0] + "_flights.json"
            if os.path.exists(json_path):
                try:
                    with open(json_path, "r", encoding="utf-8") as f:
                        self._current_flights = json.load(f)
    ...
    def _fit_to_canvas(self, img: Image.Image):
        ...
        if getattr(self, "_current_flights", None) and self._current_flights.get("flights"):
            self._draw_flight_overlays(iw, ih, cw, ch)

    def _draw_flight_overlays(self, img_w, img_h, canvas_w, canvas_h):
        import math
        flights = self._current_flights.get("flights", [])
        tlat = self._current_flights.get("target_lat")
        tlon = self._current_flights.get("target_lon")
        rad_km = self._current_flights.get("radius_km")

        if tlat is None or tlon is None or not rad_km:
            return

        km_per_pixel_w = (rad_km * 2) / float(img_w + 1)
        km_per_pixel_h = (rad_km * 2) / float(img_h + 1)
        ...
        for f in flights:
            ...
            dy_km = (tlat - flat) * 111.0
            dx_km = (flon - tlon) * (111.0 * math.cos(math.radians(tlat)))

            px_x = (img_w / 2) + (dx_km / km_per_pixel_w)
            px_y = (img_h / 2) + (dy_km / km_per_pixel_h)

            cx = (canvas_w / 2) + ((px_x - img_w/2) * scale)
            cy = (canvas_h / 2) + ((px_y - img_h/2) * scale)

            cr = 6
            self._canvas.create_oval(cx - cr, cy - cr, cx + cr, cy + cr,
                                     outline="yellow", width=2)
            lbl = f"{f.get('callsign', '')} {f.get('altitude_m', 0):.0f}m"
            self._canvas.create_text(cx + 10, cy, text=lbl, fill="yellow", anchor=tk.NW)
